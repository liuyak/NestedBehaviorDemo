 ...
 */
public class SNBBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public abstract static class BottomSheetCallback {
         ...
         */
        public abstract void onSlide(@NonNull View bottomSheet, float slideOffset, boolean slideByOutsideView);
    }
    ...
    private NestedScrollFlingRunnable nestedScrollFlingRunnable = null;
    ...
    private boolean slideByOutsideView = false;

    private NestedRecycleView snbNestedRecycleView;
    ...
    public void setSlideByOutsideView(boolean slideByOutsideView) {
    ...
    public void setOutsideView(NestedRecycleView outsideView) {
        this.snbNestedRecycleView = outsideView;
    }

    public SNBBottomSheetBehavior() {
    }

    public SNBBottomSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void scrollOutsideView(int dy) {
        if (snbNestedRecycleView != null && slideByOutsideView) {
            snbNestedRecycleView.setScrollByOutsideView(true);
            snbNestedRecycleView.scrollBy(0, dy);
        }
    }

    public void setSlideHeight(int slideHeight) {
        if (slideHeight <= peekHeight) {
            slideHeight = peekHeight;
            setStateInternal(STATE_COLLAPSED);
        } else if (slideHeight >= parentHeight) {
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !callbacks.isEmpty()) {
            ...
            for (int i = 0; i < callbacks.size(); i++) {
                callbacks.get(i).onSlide(bottomSheet, slideOffset, slideByOutsideView);
            }
        }
    }

    private void handleViewDragHelperFling(View child, float velocityY) {
        if (viewDragHelper != null) {
            if (velocityY < 0) {
                viewDragHelper.flingCapturedView(0, 0, 0, collapsedOffset);
            } else {
                viewDragHelper.flingCapturedView(0, parentHeight - collapsedOffset, 0, parentHeight - peekHeight);
            }

            if (flingRunnable == null) {
                flingRunnable = new FlingRunnable(child);
            }
    ...
    private boolean nestedScrollFling = false;

    private void startNestedScrollFling(View child, float velocityY) {
        velocityY *= -1; // 跟ViewDragHelper 里面的velocityY 正负值相反
        if (velocityY < 0) {
            overScroller.fling(child.getLeft(), child.getTop(),
                    0,
                    (int) velocityY,
                    0, 0, 0, collapsedOffset);
        } else {
            overScroller.fling(child.getLeft(), child.getTop(),
                    0,
                    (int) velocityY,
                    0, 0, parentHeight - collapsedOffset, parentHeight - peekHeight);
        }

        nestedScrollFling = true;

        if (nestedScrollFlingRunnable == null) {
            nestedScrollFlingRunnable = new NestedScrollFlingRunnable(child);
        }
    ...
    private class NestedScrollFlingRunnable implements Runnable {

        private final View view;

        private boolean isPosted;

        NestedScrollFlingRunnable(View view) {
            this.view = view;
        }
    ...
    private boolean continueNestedScrollFling(View view) {
        if (nestedScrollFling) {
            boolean keepGoing = overScroller.computeScrollOffset();
            final int x = overScroller.getCurrX();
            final int y = overScroller.getCurrY();
            final int dx = x - view.getLeft();
            final int dy = y - view.getTop();
            ...
            if (keepGoing && x == overScroller.getFinalX() && y == overScroller.getFinalY()) {
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcelable superState, @NonNull SNBBottomSheetBehavior<?> behavior) {
            super(superState);
        ...
        /**
         * This constructor does not respect flags: {@link SNBBottomSheetBehavior#SAVE_PEEK_HEIGHT}, {@link
         * SNBBottomSheetBehavior#SAVE_FIT_TO_CONTENTS}, {@link SNBBottomSheetBehavior#SAVE_HIDEABLE}, {@link
         * SNBBottomSheetBehavior#SAVE_SKIP_COLLAPSED}. It is as if {@link SNBBottomSheetBehavior#SAVE_NONE}
         * were set.
         *
         * @deprecated Use {@link SavedState( Parcelable , SNBBottomSheetBehavior )} instead.
         */
        @Deprecated
    ...
    /**
     * A utility function to get the {@link SNBBottomSheetBehavior} associated with the {@code view}.
     *
     * @param view The {@link View} with {@link SNBBottomSheetBehavior}.
     * @return The {@link SNBBottomSheetBehavior} associated with the {@code view}.
     */
    @NonNull
    @SuppressWarnings("unchecked")
    public static <V extends View> SNBBottomSheetBehavior<V> from(@NonNull V view) {
        ...
        if (!(behavior instanceof SNBBottomSheetBehavior)) {
            throw new IllegalArgumentException("The view is not associated with SNBBottomSheetBehavior");
        }
        return (SNBBottomSheetBehavior<V>) behavior;
    }
